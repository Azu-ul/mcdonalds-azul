...
router.post('/addresses', authenticateToken, async (req, res) => {
  try {
    ...
    // Si es_default, desactivar otras direcciones predeterminadas
    if (is_default) {
      await pool.query(
    ...
    const [result] = await pool.query(`
      ...
      VALUES (?, ?, ?, ?, ?)
    `, [req.user.id, address.trim(), latitude || null, longitude || null, is_default || 0]);
 ...
 * @desc    Actualizar dirección guardada
 * @access  Private
 */
 ...
 * @desc    Eliminar dirección guardada
 * @access  Private
 */
